					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						...
						<TABLE cellSpacing=0 cellPadding=5  border=0>
							<TR>
								...
								<td>客户级别 ：</td>
								<td>
								 <select name="custLevel.did">
							 		<c:forEach items="${dicts }" var="dict">
							 			<c:if test="${dict.did eq customer.custLevel.did}">
							 				<option value="${dict.did }">${dict.dname }</option>
							 			</c:if>
							 		</c:forEach>
								 	<c:forEach items="${dicts }" var="dict">
							 			<c:if test="${dict.did ne customer.custLevel.did}">
							 				<option value="${dict.did }">${dict.dname }</option>
							 			</c:if>
							 		</c:forEach>
								 </select>
							...
							<TR>
								...
								<td>
								<INPUT class=textbox id=sChannel2
														style="WIDTH: 180px" maxLength=50 name="custZip" value="${customer.custZip }">
								</td>
							</TR>
							<TR>
								<td>客户传真 ：</td>
